    ...
    for attribute_output in attribute_output_raw:
        ...
        for analysis_type in ['Sent', 'Doc']:
            ...
            for keyword_input, example_input in [i for i in product([False],[True])]:
                ...
                df_temp.loc[:,f"Raw_Output_LLM_Events_{col_suffix}_evidence_{analysis_type}"] = df_temp[f"LLM_Events_{col_suffix}_evidence_{analysis_type}"].apply(lambda x: x['raw_output'])
                ...
                gt_file = f"../exports/04b_groundtruth/P-SET/Annotated/{ET}_{focus_type}.pkl"
